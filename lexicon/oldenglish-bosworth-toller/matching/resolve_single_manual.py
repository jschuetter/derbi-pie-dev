...
input_file = 'oe_unique_matches.csv'

remaining_file_path = 'oe_unique_remaining.csv'
if os.path.exists(remaining_file_path): 
    print("Do you want to start from oe_unique_remaining.csv?")
    ch = getch()
    if ch in ("y", "\r", "\n"):
...
try:
    ...
    with open(f'unique_manual_approved_{time.time()}.csv', 'w') as appfile: 
        writer = csv.DictWriter(appfile, FIELDNAMES)
        writer.writeheader()
        writer.writerows(rows_approved)
    with open(f'unique_manual_rejected_{time.time()}.csv', 'w') as rejfile: 
