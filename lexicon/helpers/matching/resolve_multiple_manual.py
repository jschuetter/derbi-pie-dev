...
input_file = 'temp/duplicate_matches.csv'

remaining_file_path = 'temp/multiple_remaining.csv'
if os.path.exists(remaining_file_path): 
    print("Do you want to start from temp/multiple_remaining.csv?")
    ch = getch()
    if ch in ("y", "\r", "\n"):
...
with open(input_file, 'r') as infile:
    ...
    try:
        ...
        with open(f'approved/multiple_manual_{time.time()}.csv', 'w') as appfile: 
            writer = csv.DictWriter(appfile, FIELDNAMES)
            writer.writeheader()
            writer.writerows(approved_matches)
        with open(f'rejected/multiple_manual_unmatched_{time.time()}.csv', 'w') as rejfile: 
